...
namespace RoboShowdown.UI.Battleground
{
    public class BattleGrid : Grid
    {
        ...
        public void Reset(int width, int height, ICollection<IBattlegroundObject> objects)
        {
            ...
            this.Children.Clear();
            objects.Select(w => new { ViewModel = w, Control = selector.GetControlForViewModel(w) }).ToList().ForEach(w =>
            {
